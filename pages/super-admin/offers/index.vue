...
const offersResults = ref<ISuperOffersListTableRow[]>([]);
...
const onDataRequest = (offerId: string) => {
  const [filteredOffer] = offersResults.value.filter(
    (offer) => offer.id === Number(offerId)
  );
  const [company] = companiesResult.value.filter(
    (company) => company.key === Number(companyId.value)
  );
  if (filteredOffer) {
    return {
...
const fetchCompanyOffers = async (companyId: number) => {
  ...
  if (error.value) {
    ...
    offersResults.value = data.value;
